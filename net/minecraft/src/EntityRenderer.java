...
public class EntityRenderer
{
    ...
    private void renderHand(float par1, int par2)
    {
        if (this.debugViewDirection <= 0)
        {
            ...
            if (!Reliant.getInstance().getModuleHelper().freeCam.getState() && this.mc.gameSettings.thirdPersonView == 0 && !this.mc.renderViewEntity.isPlayerSleeping() && !this.mc.gameSettings.hideGUI && !this.mc.playerController.enableEverythingIsScrewedUpMode())
            {
    ...
    public void updateCameraAndRender(float par1)
    {
        ...
        if (this.mc.inGameHasFocus && var2)
        {
            ...
            if (this.mc.gameSettings.smoothCamera)
            {
                ...
                if(Reliant.getInstance().getModuleHelper().freeCam.getState()) {
                	Reliant.getInstance().getModuleHelper().freeCam.freecamEntity.setAngles(var5, var6 * (float)var7);
                } else {
    ...
    public void renderWorld(float par1, long par2)
    {
        ...
        for (int var13 = 0; var13 < 2; ++var13)
        {
            ...
            boolean curBobbing = this.mc.gameSettings.viewBobbing;
            this.mc.gameSettings.viewBobbing = false;
            this.setupCameraTransform(par1, var13);
            Reliant.getInstance().getEventManager().fireEvent(new EventRender());
            this.mc.gameSettings.viewBobbing = curBobbing;
    ...
    protected void renderRainSnow(float par1)
    {
        ...
        EventRainSnow event = new EventRainSnow();
        Reliant.getInstance().getEventManager().fireEvent(event);
